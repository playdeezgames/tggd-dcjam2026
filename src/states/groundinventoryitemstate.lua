...
local function createMenuHandler(state)
    ...
    result:addItem(menuitem.create(M.TAKE,"Take", colors.WHITE, colors.BLACK))
...
local function itemHandler(state,menuItemId)
    ...
    elseif menuItemId == M.TAKE then
        local i = w:getItem(avatar:getYoke(yokes.ITEM_ID))
        avatar:setYoke(yokes.ITEM_ID, nil)
        avatar:getLocation():removeItem(i)
        avatar:addItem(i)
    end
